...
void __fastcall TMainForm::Panel211Resize(TObject *Sender)
{
	...
	int hSol,ha,h1,h2,h3,henu;
	hSol=PanelSol->Height;
	h2=hSol/2-h/2;
	h1=h2-h-12;
	ha=h1-h-12;
	h3=h2+h+12;
	henu=h3+h+12;
	PlabelA ->Top=ha; Solution ->Top=ha; IndSol ->Top=ha;//16
	Plabel1 ->Top=h1; Pos1 ->Top=h1;                     //40
	Plabel2 ->Top=h2; Pos2 ->Top=h2;                     //64
	Plabel3 ->Top=h3; Pos3 ->Top=h3;                     //88
	LabelNEU->Top=henu;                                  //112

	PlabelA ->Left=15; Solution ->Left=60; IndSol ->Left=100;
	Plabel1 ->Left=15; Pos1 ->Left=60;
	Plabel2 ->Left=15; Pos2 ->Left=60;
	Plabel3 ->Left=15; Pos3 ->Left=60;
	LabelNEU->Left=15;
	...
	int hRest,hage,hrto,hns,hvar;
	hRest=PanelRest->Height;
	hrto=hRest/2-6-h;
	hage=hrto-h-12;
	hns=hrto+h+12;
	hvar=hns+h+12;
	LabelAge  ->Top=hage;    //18
	LabelRatio->Top=hrto;    //42
	LabelNSat ->Top=hns;     //66
	LabelVar  ->Top=hvar;    //90
...
//	int h2=abs(LabelNEU->Height),w2=PanelSol->Width;
//	int y0,y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,y11,y12,y13,y14;
//	trace(3,"Panel211Resize\n");
////	y2=Panel211->Height/2-h/2-6;
//	y2=Panel211->Height/2-h/2-20;
//	y1=y2-h-10; y3=y2+h+10; y0=y1-h-12; y4=y3+h+10;
//	y9=PanelSol->Height/2-h/2;
//	y8=y9-h-10;y7=y8-h-10;y6=y7-h-10;y5=y6-h-10;
//	y10=y9+18;y11=y10+18;y12=y11+18;y13=y12+18;y14=y13+18;
////	y6=y5+18;y7=y6+18;y8=y7+18;y9=y8+18;y10=y9+18;y11=y10+18;
...
//	LabelHPL->Top=y7;
//	LabelVPL->Top=y8;
//	LabelHA ->Top=y9;  HAState ->Top=y9;
//	LabelVA ->Top=y10; VAState ->Top=y10;
//	LabelAge ->Top=y11;
//	LabelRatio ->Top=y12;
//	LabelNSat->Top=y13;
//	LabelVar->Top=y14;
...
//	PanelSol ->Left=14; PanelSol ->Width=w-42;
//	LabelStd ->Left=2; LabelStd ->Width=w2-4;
//	LabelNEU ->Left=3; LabelNEU ->Width=w2-4;
//	LabelAge ->Left=2; LabelAge ->Width=w2-4;
//	LabelRatio ->Left=2; LabelRatio ->Width=w2-4;
//	LabelNSat->Left=2; LabelNSat->Width=w2-4;
//	LabelHPL->Left=2; LabelHPL->Width=w2-4;
//	LabelVPL->Left=2; LabelVPL->Width=w2-4;
//	LabelHA->Left=2;  HAState->Left=60; HAState->Width=w2-42;         //LabelHA->Width=w2-4;
//	LabelVA->Left=2;  VAState->Left=60; VAState->Width=w2-42;         //LabelVA->Width=w2-4;
//	LabelVar->Left=2; LabelVar->Width=w2-4;
//	UpdatePos();
}
...
void __fastcall TMainForm::BtnPlotClick(TObject *Sender)
{
    ...
    if (!ExecCmd(cmd,1)) {
        ShowMessage("error : plot execution");
    }
}
...
void __fastcall TMainForm::SvrStart(void)
{
	...
	/*lyj add*/
	PrcOpt.RefRovxyz[0]=RovPos[0];
	PrcOpt.RefRovxyz[1]=RovPos[1];
	PrcOpt.RefRovxyz[2]=RovPos[2];
...
{
    AnsiString label[]={
		"纬度/经度/高程","纬度/经度/高程","X/Y/Z-空间直角坐标","东/北/天-基线",
		"俯仰/航向/长度-基线",""
    };
...
//	UnicodeString s_ch[8];
	...
	if (SolType==0) {
		...
		s[2]=SolOpt.height==1?"大地高:":"椭球高:";
//		s[3].sprintf(L"%.0f\u00B0%s %02.0f' %07.4f\"",fabs(dms1[0]),CHARDEG,dms1[1],dms1[2]);
		...
		s[0]=pos[0]<0?"S:":"纬    度:"; s[1]=pos[1]<0?"W:":"经    度:";
		s[2]=SolOpt.height==1?"大地高:":"椭球高:";
		s[3].sprintf(L"%.8f \u00B0",fabs(pos[0])*R2D);
		s[4].sprintf(L"%.8f \u00B0",fabs(pos[1])*R2D);
		s[5].sprintf(L"%.3f m",pos[2]);
...
//		s[6].sprintf(L"标准差  E:%6.3f   N:%6.3f   U:%6.3f m",SQRT(Qe[0]),SQRT(Qe[4]),SQRT(Qe[8]));
		...
		s[3].sprintf(L"%.3f \u00B0",pitch*R2D);
		s[4].sprintf(L"%.3f \u00B0",yaw*R2D);
...
//		s[6].sprintf(L"标准差  E:%6.3f   N:%6.3f   U:%6.3f m",SQRT(Qe[0]),SQRT(Qe[4]),SQRT(Qe[8]));
	...
	for (i=9;i<11;i++) {
		label[i]->Font->Color=s[i]=="正常"?clGreen:clRed;
	}
...
void __fastcall TMainForm::DrawPlot(TImage *plot, int type, int freq)
{
    ...
    if (type==0) { // snr plot rover+base
        if (w<=3*h) { // vertical
			...
			s1=L"流动站:基准站 "+fstr[freq]+L" 信噪比 (dBHz)";
            DrawText(c,x,1,s1,clGray,1,2);
        }
		else { // horizontal
            DrawSnr(c,w/2,h-tm,0  ,tm,0,freq);
			DrawSnr(c,w/2,h-tm,w/2,tm,1,freq);
			s1=L"流动站 "+fstr[freq]+L" 信噪比 (dBHz)";
			s2=L"基准站 "+fstr[freq]+L" 信噪比 (dBHz)";
			DrawText(c,x,1,s1,clGray,1,2);
        ...
        DrawBL(plot,w,h);
		DrawText(c,x,1,L"基线",clGray,1,2);
    }
