...
void __fastcall TMonitorDialog::Timer2Timer(TObject *Sender)
{
	...
	if (TypeF>=17) {  //输出流和错误信息
		...
		for (i=0;i<len;i++) {
			...
			if (rtcm.msgtype[0]&&cmccin==0) {
				n=sprintf(buff,"%s\n",rtcm.msgtype);
				AddConsole((uint8_t *)buff,n,1);
				rtcm.msgtype[0]='\0';
			}
			else if (rtcm.cmccmsgtype[0]&&cmccin) {    //单独输出完好性电文信息
				n=sprintf(buff,"%s\n",rtcm.cmccmsgtype);
				AddConsole((uint8_t *)buff,n,1);
				rtcm.cmccmsgtype[0]='\0';
			}
	    }
	}
	else if (ConFmt<17) {
...
void __fastcall TMonitorDialog::SetSat(void)
{
	int i,j=0;
	AnsiString s,label[]={
		"卫星","状态","方位角 (deg)","高度角 (deg)","LG (m)","PHW(cyc)",
...
{
	AnsiString label[]={
		"STR","数据流","类型","格式","模式","状态","输入(bytes)","输入(bps)",
		"输出(bytes)","输出(bps)","路径","消息"
	};
...
{
	AnsiString ch[]={
		"流动站输入","基准站输入","改正信息输入","解1输出",
		"解2输出","流动站日志","基准站日志","改正信息日志",
		"监视"
	};
	AnsiString type[]={
