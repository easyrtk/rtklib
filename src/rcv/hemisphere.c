...
#define  SBS_MAX_NO     44   /* 由24改为44: 120~158,183~187 */
#define  QZS_MAX_NO     20
#define  GAL_MAX_NO     36   //由20改为30
...
#define GPS_L5_MAX  21  /* 避免优先级调整导致函数错误 */
...
static int UnifySignalType(int nSystem, BYTE bPcode, int *nSignalType, int *codeType)
{
	...
	if (nSystem == GPS_SYSTEM)
	{
		...
		{
			*nSignalType = GPS_C2S;  /* 交换L2观测类型?? *///将GPS_C2C修改为GPS_C2X修改[2019/09/04]
			nFreq = 2;
			*codeType = 16;
		}
		else if (*nSignalType == 3)
		{
			*nSignalType = GPS_C5Q;//将GPS_C5I修改为GPS_C5Q [2019/09/04]
			nFreq = 3;
			*codeType = 25;
		}
	...
	else if (nSystem == GAL_SYSTEM)
	{
		...
		if (*nSignalType == 0)
		{
            *nSignalType = GAL_C1X;//将GAL_C1C修改为GAL_C1X [2019/10/15]
			nFreq = 0;
            *codeType = 12;
		}
		else if (*nSignalType == 1)
		{
            *nSignalType = GAL_C5X;//将GAL_C5Q改为GAL_C5X [2019/10/15]
			nFreq = 1;
            *codeType = 26;
		}
		else if (*nSignalType == 2)
		{
            *nSignalType = GAL_C7X;//将GAL_C7Q改为GAL_C7X [2019/10/15]
			nFreq = 2;
            *codeType = 29;
		}
		else if (*nSignalType == 3)//增加[2019/05/30]
		{
            *nSignalType = GAL_C6C;
			nFreq = 3;
            *codeType = 60;
		}
		else if (*nSignalType == 4)//增加[2019/05/30]
		{
            *nSignalType = GAL_C8Q;
			nFreq = 4;
            *codeType = 39;
	...
	else if (nSystem == QZS_SYSTEM)
	{
		...
		{
            *nSignalType = QZS_C2X; /* L2C *///将QZS_C2S改为QZS_C2X [2019/09/04]
			nFreq = 1;
            *codeType = 18;
		}
		else if (*nSignalType == 3)
		{
            *nSignalType = QZS_C5Q; /* L5 *///将QZS_C5I转换为QZS_C5Q[2019/09/04] 
			nFreq = 2;
            *codeType = 25;
		}
		//else if (*nSignalType == 4)
...
static double GetWaveLength(int iSystem, int iSignalType, int iFreq /* = InvalidFreq */)
{
	...
	else if (iSystem == GLO_SYSTEM)
	{
		...
		else if (iSignalType >= GLO_G1a_MIN && iSignalType <= GLO_G1a_MAX)  //304格式
		{
...
void GetObsValue(int nSystem, SObsPacket item, int nSignal, Hemis_MeasBlock *Obs)
{
	...
	{
		Obs->dSNR = 0.0;  //对数函数的定义域必须大于0,否则不是数据
	}
	...
	if (!(item.P7_Doppler_FL >> 24 & 0x1)) //bit24:1为负，0为正还是反之？
	{
...
/* 参考相位提供高位、输入相位低23位，组合后的新相位与参考相位差值控制在4196 */
double AdjustPhase(double dFinePhase, double dTotalPhase)
{
	...
	dNewPhase = nTem / 1024.0;  //修改相位[2014/12/31 GaoWei]


	////调整组合相位和参考相位的差值，使之小于4096，调整后的值即为实际相位
...
extern int readGnssObs(unsigned char* buff, int nStart, obs_t *obs, int *iPagesTotal, int *iPageNumber, int *nIndex)
{
	...
	*iPageNumber = (item.m_uPageCount >> 22 & 0x3F);
	...
	for (int i = 0; i < CHANNELS_gen; i++)
	{
		...
		double dRoughPsr = (item.m_aulCodeMSBsPRN[i] >> 13 & 0x7FFFF)*256.0; /* 伪距高19位 */
		if (fabs(dRoughPsr) < 1E-6)
		{
			continue;
		}

		int nSignalType = item.m_awChanSignalSYS[i] >> 4 & 0xF;    /* 原始信号id */
		BYTE bPcode = item.m_awChanSignalSYS[i] >> 13 & 0x1;  /* glonass是否为p码 */

		int codeType = 0;
		int nL = UnifySignalType(nSystem, bPcode, &nSignalType, &codeType);      /* 转换为统一的信号id */
		...
		double lam = GetWaveLength(nSystem, nSignalType, nFreq);     /* 卫星波长 */
...
static int readGalileoEph(unsigned char* buff, int nStart, nav_t *nav)
{
	...
	eph.tgd[1] = getSignData(SF3words[5] >> 10 & G10BITS, 10, TRUE)*pow(2.0, -32);//修改,将tgd1与tgd2互换[2019/06/28]
	...
	if (E1BDVS)
	{
		Health |= eGALILEO_HEALTH_E1B_DV;   // 不确定各数据位有效性是否需要取反?
	}
	if (E1BH)
	{
...
void GetCalendarTime(int N4, int Nt, double sec, GPSTime *timeMsg)
{
	int J = 0;
	int nDay = 0;
	if (Nt <= 366)   //[2015/1/7 GaoWei]包括当天计算在内，故取等号
	{
		J = 0;
		nDay = Nt;
	}
...
int readGlonassString(ULONG asStrings[][3], int size, geph_t *item)
{
	...
	for (int i = 0; i < size; ++i)
	{
		...
		else if (nTem == 2)//GLONASS_String 2
		{
			...
			item->svh = nTem >> 2 & 0x1;  //只取最高位，后2位不考虑
